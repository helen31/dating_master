    ...
    <section class="panel">
        <header class="panel-heading">
            ...
            @if(Auth::user()->hasRole('Moder') || Auth::user()->hasRole('Owner'))
                @if($user->status_id == 2)
                    <a href="{{ url(App::getLocale().'/admin/man/activate/'.$user->id) }}" class="btn btn-success btn-xs" >
                        <i class="fa fa-arrow-up"></i>  Активировать
                    </a>
                @else
                    <a href="{{ url(App::getLocale().'/admin/man/deactivate/'.$user->id) }}" class="btn btn-warning btn-xs" >
                        <i class="fa fa-pause"></i>  Приостановить
                    </a>
                @endif
            @endif
            <a href="{{ url(App::getLocale().'/admin/man/drop/'.$user->id) }}" class="btn btn-danger btn-xs" >
                            ...
                            <div class="avatar_block col-md-12">
                                ...
                                <div class="col-md-8">
                                    <div class="form-group col-md-12">
                                        {!! Form::label('vip', 'VIP') !!}
                                        <input type="checkbox" name="vip" {{ ($user->vip == 0) ?: 'checked' }}>
                                    </div>
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        {!! Form::text('birthday', $user->profile->birthday, ['class'=>'form-control', 'id'=>'datepicker-birthday3', 'placeholder'=>'yyyy-mm-dd']) !!}
                                    </div>
...
@section('scripts')
    ...
    <script>
        ...
        function deleteProfileFoto(event,foID){
            ...
            $.post( "/admin/men/dropProfileFoto/"+foID, {_token : $('input[name="_token').val()} ).done( function( data ) {
                $("#photo-"+foID).remove();
            });
        }
    </script>
